...
#diffrence from haplism github
#1.convert all tab to 4 spaces using pycharm crl+shif+a > type space 
#2. add             print "Mean of block length: {0}".format(np.mean(Total_common_variants) / 2)
...
Comparison is not possible unless -t, --true flag is on!")
 ...
 for meaningful comparisons!")
        ...
        if 1: #not true_hap:         # between the true homologues resulting in the estimated homologues.
            #if verbose:
...
\nPerfect Solution Rate for common variants (including variants with wrong dosages): {1}".format(round(sum(x*y for x,y in zip(Total_corr,Total_common_variants))/(1e-10+sum(z for z in Total_common_variants)),3),
            ...
            print "Mean of block length: {0}".format(np.mean(Total_common_variants) / 2)
