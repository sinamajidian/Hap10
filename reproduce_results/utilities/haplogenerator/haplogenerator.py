...
This is a revised version by sina majidian 
1/1/2019
...
def rnd_position(seq, rates, model, mean_logs, sd_logs, sample=None):
	...
	try:
		N_positions=[pos for pos, char in enumerate(seq) if char == 'N']  # finding position of N in the seq, it will be used in  while _n<contig_length:
		...
		if model==1: 				# Specify mutation/indel points applying a poisson process
			if rates[0]<0:
				...
				while _n<contig_length:
					...
					if _n<contig_length and (_n not in N_positions):
						rndmupos.append(_n)		
			_rate=None
			if rates[1]<0:
...
Missing probabilities will be set equally, so that the total sum of dosage probabilities becomes one!" %len(dosage)
 ...
 Missing probabilities will be set to zero!" %len(dosage)
			...
			if _random:											# The code for random mode starts here
				with open(outputfile.name,'w') as out_handle, open(outindexfile.name,'w') as out_index:		
					for i, l in enumerate(length):
						...
						out_handle.write('>'+contig_id+'\n')#+os.linesep
						out_handle.write(str(contig)+os.linesep) ##((os.linesep).join(wrap(str(contig),width=linelen))+os.linesep)
						if verbose:
		...
		convert coordinate\nand length strings to an integer!",)
						...
						out_handle.write('>'+_descrip+'\n') #+os.linesep
						out_handle.write(str(_new_seq)+os.linesep) # ((os.linesep).join(wrap(str(_new_seq),width=ref_linelen))+os.linesep)
						if verbose:
